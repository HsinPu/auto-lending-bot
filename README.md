...
- `AUTO_REBALANCE_OPEN_OFFERS=true` and `AUTO_CANCEL_OPEN_OFFERS=true` are the defaults, so the bot syncs open offers before each run and cancels stale offers for repricing when live mode is explicitly enabled. With `STALE_OFFER_REPRICE_ENABLED=true`, the bot waits roughly 15 minutes for `fast`, 60 minutes for `balanced`, or 240 minutes for `yield` before canceling an unfilled offer for repricing. `STALE_OFFER_REPRICE_DEBOUNCE_MINUTES` prevents repeated repricing cancels for the same currency, and `STALE_OFFER_REPRICE_MAX_CANCELS_PER_RUN` caps how many stale offers can be canceled in one run.
